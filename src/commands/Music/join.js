  ...
  run: async (client, message, args, prefix, player) => {
      ...
      if (botChannel.id === userChannel.id) {
        return message.reply({
          embeds: [
            new EmbedBuilder()
              .setColor("#00ff55")
              .setDescription(`<:check:1465237660301004884> I'm already connected to **${botChannel.name}**.`),
          ],
        });
      } else {
        return message.reply({
    ...
    if (!node) {
      return message.reply({
        embeds: [
          new EmbedBuilder()
            .setDescription("<:cross:1465237676788813864> No Lavalink nodes are available!")
    ...
    return message.reply({
      embeds: [
        new EmbedBuilder()
          .setColor("#00ff55")
          .setDescription(`<:check:1465237660301004884> | Joined **${userChannel.name}** successfully.`),
      ],
    });
  },
};