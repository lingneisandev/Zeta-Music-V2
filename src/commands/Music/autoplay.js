  ...
  run: async (client, message, args, prefix, player) => {
    try {
      ...
      if (player.isAutoplay) {
        ...
        const embed = new EmbedBuilder()
          .setDescription("<:check:1465237660301004884> | *Autoplay has been:* `Deactivated`")
          .setColor(client.color);
        return message.reply({ embeds: [embed] });
      } else {
        if (!player.current) {
          const embed = new EmbedBuilder()
            .setDescription("<:cross:1465237676788813864> | No track is currently playing. Play a track first to enable autoplay.")
            .setColor(client.color);
          return message.reply({ embeds: [embed] });
        }
        ...
        const embed = new EmbedBuilder()
          .setDescription("<:check:1465237660301004884> | Autoplay has been: `Activated`")
          .setColor(client.color);
