...
// Owner IDs
const ownerIDS = ['1383706658315960330', '1409859901488103516'];
  ...
  run: async (client, message, args, prefix) => {
    ...
    try {
      ...
      if (args[0].toLowerCase() === "add") {
        if (!userId) {
          return message.channel.send({
            embeds: [new EmbedBuilder().setColor("Red").setDescription("<:cross:1465237676788813864> Please mention a user or provide a valid ID to add.")]
          });
        }
      ...
      if (args[0].toLowerCase() === "remove") {
        if (!userId) {
          return message.channel.send({
            embeds: [new EmbedBuilder().setColor("Red").setDescription("<:cross:1465237676788813864> Please mention a user or provide a valid ID to remove.")]
          });
        }
        ...
        if (!data) {
          return message.channel.send({
            embeds: [new EmbedBuilder().setColor("Orange").setDescription("<:cross:1465237676788813864> This user does not have NoPrefix.")]
          });
        }
      ...
      if (["list", "show"].includes(args[0].toLowerCase())) {
        ...
        if (!data.length) {
          return message.channel.send({
            embeds: [new EmbedBuilder().setColor("Grey").setDescription("<:cross:1465237676788813864> No users currently have NoPrefix.")]
          });
        }
