  ...
  run: async (client, message, args) => {
    ...
    if (!channel || !channel.isTextBased() || !channel.viewable) {
      const errorEmbed = new EmbedBuilder()
        .setColor(client.color)
        .setDescription("<:cross:1465237676788813864> Please provide a valid text channel.");
    ...
    try {
      if (action === "add") {
        ...
        if (exists) {
          const alreadyIgnoredEmbed = new EmbedBuilder()
            .setColor(client.color)
            .setDescription(`<:cross:1465237676788813864> ${channel} is already ignored.`);
        ...
        const successEmbed = new EmbedBuilder()
          .setColor(client.color)
          .setDescription(`<:check:1465237660301004884> ${channel} has been added to the ignore list.`);
        ...
        if (!exists) {
          const notIgnoredEmbed = new EmbedBuilder()
            .setColor(client.color)
            .setDescription(`<:cross:1465237676788813864> ${channel} is not in the ignore list.`);
        ...
        const successEmbed = new EmbedBuilder()
          .setColor(client.color)
          .setDescription(`<:check:1465237660301004884> ${channel} has been removed from the ignore list.`);
        return message.channel.send({ embeds: [successEmbed] });
      }
    } catch (error) {
      console.error(error);
      const errorEmbed = new EmbedBuilder()
        .setColor(client.color)
        .setDescription("<:cross:1465237676788813864> An error occurred while processing the command.");
      return message.channel.send({ embeds: [errorEmbed] });
    }
  },
};